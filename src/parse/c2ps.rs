...
impl C2PS {
    ...
    pub fn get_ps_min_max(&self) -> (ScreenUnit, ScreenUnit, ScreenUnit, ScreenUnit) {
        ...
        let (mut min_x, mut min_y, mut max_x, mut max_y) = (
            self.regions[0].top_left_ps.x,
            self.regions[0].top_left_ps.y,
            self.regions[0].top_left_ps.x,
            self.regions[0].top_left_ps.y,
        );
...
fn convert(r: &Region, c: &Coordinate) -> Option<[PointScreen; 4]> {
    if c.x >= r.top_left_c.x
        && c.x < (r.top_left_c.x + r.wh_c.x)
        && c.y >= r.top_left_c.y
        && c.y < (r.top_left_c.y + r.wh_c.y)
    {
        return Some([
            PointScreen {
                x: r.top_left_ps.x + r.cell_wh_ps.x * (c.x - r.top_left_c.x),
                y: r.top_left_ps.y + r.cell_wh_ps.y * (c.y - r.top_left_c.y),
            },
            PointScreen {
                x: r.top_left_ps.x + r.cell_wh_ps.x * (c.x - r.top_left_c.x),
                y: r.top_left_ps.y + r.cell_wh_ps.y * (c.y - r.top_left_c.y + 1),
            },
            PointScreen {
                x: r.top_left_ps.x + r.cell_wh_ps.x * (c.x - r.top_left_c.x + 1),
                y: r.top_left_ps.y + r.cell_wh_ps.y * (c.y - r.top_left_c.y + 1),
            },
            PointScreen {
                x: r.top_left_ps.x + r.cell_wh_ps.x * (c.x - r.top_left_c.x + 1),
